...
class TestRemoveCommand:
    ...
    def test_remove_no_matching_permissions(self, monkeypatch, tmp_path: Path):
        ...
        assert "Microsoft.Storage/storageAccounts/read" in manager.current_role.Permissions[0].Actions
...
class TestViewCommand:
    ...
    def test_view_with_all_flag(self, monkeypatch, tmp_path: Path):
        ...
        permissions_list = [f"Microsoft.ResourceType{i}/resource/action" for i in range(15)]
        role = manager.create_role("Large Role", "Role with many permissions")
        role.Permissions = [
            PermissionDefinition(Actions=permissions_list)
        ]
        manager.current_role = role
...
class TestPublishCommand:
    ...
    def test_publish_with_subscription_id(self, monkeypatch, tmp_path: Path):
        ...
        mock_client_class = Mock()
        mock_client_instance = Mock()
        mock_client_instance.create_custom_role.return_value = {"id": "role-123", "name": "Publish Test"}
        mock_client_class.return_value = mock_client_instance
    ...
    def test_publish_azure_error(self, monkeypatch, tmp_path: Path):
        ...
        mock_client_class = Mock()
        mock_client_instance = Mock()
        mock_client_instance.create_custom_role.side_effect = Exception("Azure API Error")
        mock_client_class.return_value = mock_client_instance
...
class TestCreateCommand:
    ...
    def test_create_initializes_empty_permissions(self, monkeypatch, tmp_path: Path):
        ...
        result = runner.invoke(
            cli.cli,
            ["create", "--name", "Empty Role", "--description", "No perms yet"]
        )
    ...
    def test_create_with_special_characters(self, monkeypatch, tmp_path: Path):
        ...
        result = runner.invoke(
            ...
            [
                "create",
                "--name", "Role-With_Special.Chars (Test)",
...
class TestSaveCommand:
    ...
    def test_save_without_overwrite_flag_fails_if_exists(self, monkeypatch, tmp_path: Path):
...
class TestListCommand:
    ...
    def test_list_specific_role_by_name(self, monkeypatch, tmp_path: Path):
        ...
        role1 = AzureRoleDefinition(Name="Storage Admin", Description="D1", Permissions=[PermissionDefinition()])
        role2 = AzureRoleDefinition(Name="Compute Admin", Description="D2", Permissions=[PermissionDefinition()])
    ...
    def test_list_with_custom_role_dir(self, monkeypatch, tmp_path: Path):
        ...
        role = AzureRoleDefinition(Name="Custom Dir Role", Description="Test", Permissions=[PermissionDefinition()])
...
class TestLoadCommand:
    ...
    def test_load_from_file_path(self, monkeypatch, tmp_path: Path):
        ...
        role = AzureRoleDefinition(Name="File Path Role", Description="Test", Permissions=[PermissionDefinition()])
    ...
    def test_load_replaces_current_role(self, monkeypatch, tmp_path: Path):
        ...
        replacement = AzureRoleDefinition(Name="Replacement", Description="Second", Permissions=[PermissionDefinition()])
