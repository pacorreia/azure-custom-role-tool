...
class PermissionFilter:
    ...
    def filter_by_string(actions: List[str], pattern: str) -> List[str]:
        ...
        regex_pattern = f"^{regex_pattern}$" if not regex_pattern.endswith(".*") else regex_pattern
    ...
    @staticmethod
    def filter_by_type(actions: List[str], permission_type: PermissionType) -> List[str]:
        """
    ...
    def extract_actions(permissions: List[Dict]) -> Dict[str, Set[str]]:
        ...
        for perm_block in permissions:
            extracted["actions"].update(perm_block.get("Actions", []))
            extracted["not_actions"].update(perm_block.get("NotActions", []))
            extracted["data_actions"].update(perm_block.get("DataActions", []))
            extracted["not_data_actions"].update(perm_block.get("NotDataActions", []))
    ...
    def merge_permission_blocks(
        ...
        block["Actions"] = list(set(block.get("Actions", []) + (new_actions.get("actions", []))))
        block["NotActions"] = list(set(block.get("NotActions", []) + (new_actions.get("not_actions", []))))
        block["DataActions"] = list(set(block.get("DataActions", []) + (new_actions.get("data_actions", []))))
        block["NotDataActions"] = list(set(block.get("NotDataActions", []) + (new_actions.get("not_data_actions", []))))
